...
internal static class SampleColumns
{
    ...
    [
        ...
        new GridColumn("code", "コード", new GridValueAccessor<SampleRow, string>(static x => x.Code)) { Width = GridColumnWidth.Absolute(96) },
        ...
        new GridColumn("difference", "在庫差異", new GridValueAccessor<SampleRow, int>(static x => x.Difference)) { Width = GridColumnWidth.Absolute(96), Alignment = TextAlignment.End },
        ...
        new GridColumn("arrival", "入荷日", new GridValueAccessor<SampleRow, string>(static x => x.ArrivalDate)) { Width = GridColumnWidth.Absolute(80), Alignment = TextAlignment.End },
        new GridColumn("updated", "更新日時", new GridValueAccessor<SampleRow, string>(static x => x.UpdatedAt)) { Width = GridColumnWidth.Absolute(200) }
    ];
    ...
    public static void RegisterSorts(GridDataView<SampleRow> rows)
    {
        ...
        rows.RegisterSort("arrival", static x => x.ArrivalDate, StringComparer.Ordinal);
        rows.RegisterSort("updated", static x => x.UpdatedAt, StringComparer.Ordinal);
